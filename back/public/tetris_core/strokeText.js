...
function get_strokeText(string, x, y, size, weight, width, space, font, color, opacity, rotation, id) {
    ...
    for (i = 0; i < len; i++) {
        ...
        o = 0;
        for (j = 0; j < c.n; j++) {
            if (typeof(c.d[o]) != "string") { o++; continue; }
            p = c.d[o];
            o++;
            a = c.d[o];
            if (p == "m") {
...
function get_textWidth(string, size, width, space, font) {
    ...
    for (var i = 0; i < len; i++) { var c = strokeFont[font][string.charAt(i)]; if (c) total += ((c.w * fw) * mg) + m; }
    return total - (m);
}
...
function get_widthText(string, width, size, fontwidth, space, font) {
    ...
    var cur = 0,
        ...
        sp = Math.max(Math.min(space, 1000), 10) / 100,
        m = ((mg * 16 * fw) * sp) - (mg * 16 * fw);
    for (var i = 0; i < len; i++) { var c = strokeFont[font][string.charAt(i)]; if (c) { cur = ((c.w * fw) * mg) + m; if ((total + cur - (m)) <= width) { total += cur; } else { break; } } else { break; } }
